...
def chatbot_view(request, id,sign):
    paml.configure(api_key = os.getenv("PAML_API_KEY", None))
    user_input_instance = post.objects.get(id=id)
    user_input = user_input_instance.article
    prompt = f"summarize this article in 5 to 6 bullet points {user_input}"
    response = paml.chat(prompt=prompt).last
